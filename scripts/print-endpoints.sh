...
echo "    - cAdvisor Metrics:   http://${PUBLIC_IP}:8080/metrics"
...
if [ -n "$MONITORING_IP" ] && [ "$MONITORING_IP" != "None" ]; then
  ...
  echo "    - Loki Ready:         http://${MONITORING_IP}:3100/ready"
  echo "    - cAdvisor Metrics:   http://${MONITORING_IP}:8080/metrics"
  echo "    - Node Exporter:      http://${MONITORING_IP}:9100/metrics"
  echo "    - Promtail Ready:     http://${PUBLIC_IP}:9080/ready"
else
...
echo "==========================================================="
echo " JDBC URLs for H2 Console login:"
echo "   Order:        jdbc:h2:mem:orderdb"
echo "   Inventory:    jdbc:h2:file:/var/lib/inventory/inventorydb"
echo "   Payment:      jdbc:h2:mem:paymentdb"
echo "   Notification: jdbc:h2:mem:notificationdb"
