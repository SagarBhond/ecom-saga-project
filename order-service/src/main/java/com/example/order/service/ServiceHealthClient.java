...
import com.example.order.logging.CorrelationIdFilter;
...
public class ServiceHealthClient {
    ...
    /**
     * Copies the current request's correlation ID (set by CorrelationIdFilter,
     * held in MDC) onto every outbound WebClient call, so the downstream
     * service's logs can be joined with this service's logs on the same
     * X-Correlation-ID.
     */
