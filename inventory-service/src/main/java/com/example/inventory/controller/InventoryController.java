...
public class InventoryController {
    ...
    public List<InventoryItem> items() {
        ...
        log.debug("GET /api/inventory/items → {} item(s)", items.size());
        return items;
    }
    ...
    public List<InventoryReservation> reservations() {
        ...
        log.debug("GET /api/inventory/reservations → {} reservation(s)", reservations.size());
        return reservations;
    }

    @GetMapping("/items/{productId}")
    public ResponseEntity<InventoryItem> getItem(@PathVariable String productId) {
        log.debug("GET /api/inventory/items/{}", productId);
        return inventoryItemRepository.findByProductId(productId)
                .map(ResponseEntity::ok)
                .orElseGet(() -> {
                    log.warn("Inventory item for product {} not found", productId);
    ...
    public ResponseEntity<InventoryItem> createItem(@RequestBody InventoryItem item) {
        ...
        log.info("Inventory item created: productId={} availableQty={}", saved.getProductId(), saved.getAvailableQty());
